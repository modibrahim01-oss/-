...
export const TILE = 1.8; // وحدة الشبكة العالمية
...
/**
 * أقصى إزاحة للنبتة عن مركز خانتها، كنسبة من البلاطة، لكل فئة.
 *
 * تخطيط الأرباع يضع كل نبتة في مركز خانتها بالضبط، فتصطفّ صفوفًا مستوية
 * كأنها مطبوعة بآلة؛ الإزاحة تكسر ذلك. وهي بقدر ما يبقى من الخانة حول
 * النبتة: الزهرة والفطر يتركان فراغًا فيتحرّكان أكثر، والشجرة تكاد تملأ
 * خانتها فتكاد لا تتحرّك — وإلا عادت الأشجار المتجاورة تتداخل.
 */
const JITTER: Record<Tier, number> = { green: 0.1, yellow: 0.16, purple: 0.14, red: 0.05 };
...
export function buildPlant(tier: Tier, slotIndex: number, detail: Detail = "hi"): THREE.Mesh {
  ...
  m.position.x = (rng() - 0.5) * 2 * JITTER[tier] * TILE;
  m.position.z = (rng() - 0.5) * 2 * JITTER[tier] * TILE;
  return m;
}
