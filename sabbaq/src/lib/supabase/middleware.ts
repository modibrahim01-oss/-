...
/**
 * المسارات التي تحتاج جلسة. كل ما عداها عام بالتصميم: الوثيقة تنصّ على أن
 * الطلاب والزوار لا يسجّلون دخولًا، وأن أي شخص يستطيع عرض أي مزرعة — وهذا
 * ما يجعل العرض على شاشات المدرسة ممكنًا.
 */
const PROTECTED_PREFIXES = ["/supervisor", "/admin"];

function envErrorResponse(missing: { name: string; label: string }[]) {
  const rows = missing.map((v) => `<li><code>${v.name}</code> — ${v.label}</li>`).join("");
...
<style>
  body{font-family:Tahoma,system-ui,sans-serif;background:#f6f1e4;color:#1b2f26;
       margin:0;padding:24px;line-height:1.9}
  .box{max-width:640px;margin:8vh auto;background:#fff;border:1px solid #e1d8bf;
       border-radius:16px;padding:24px}
  h1{font-size:20px;margin:0 0 12px}
  code{background:#f6f1e4;padding:2px 6px;border-radius:6px;direction:ltr;
       display:inline-block;font-size:13px}
  ul{padding-inline-start:20px}
  .hint{color:#8b9188;font-size:14px;margin-top:16px}
</style></head>
<body><div class="box">
