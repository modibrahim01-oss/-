...
- **التخزين المؤقت — معطَّل فعليًا اليوم.** الصفحات تُعلن `revalidate`
  (٣٠ ثانية للمزرعة، ٦٠ للصفحة العامة) لكن `getLocale()` يقرأ كوكي اللغة،
  وقراءة الكوكي تجعل Next يصنّف الصفحة `Dynamic` فيُلغى الإعلان بصمت.
  تحقّق بنفسك: `npm run build` يطبع `ƒ` أمام كل مسار، ولا `○` واحدة.
  عمليًا كل زيارة تصل إلى قاعدة البيانات. لمدرسة واحدة (٥٠٠ طالب) الحِمل
  مقبول على الخطة المجانية، لكنه ليس ما وُصف. العلاج يحتاج نقل اللغة من
  الكوكي إلى المسار (`/ar/...` و `/en/...`) — تغيير يمسّ كل الروابط، ومنها
  رابط `/tv` المحفوظ على الشاشات، فتُرك قرارًا لا تنفيذًا.
