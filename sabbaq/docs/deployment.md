# نشر سبّاق — Supabase + Netlify

دليل بالخطوات من صفر إلى موقع يعمل. الوقت المتوقّع: ٢٠–٣٠ دقيقة.

---

## ١. إنشاء مشروع Supabase

1. افتح [supabase.com/dashboard](https://supabase.com/dashboard) ← **New project**
2. الاسم: `sabbaq` · كلمة مرور قاعدة البيانات: احفظها في مكان آمن
3. **المنطقة (Region)** — اختر الأقرب لمدرستك.
   للخليج: `eu-central-1 (Frankfurt)` عادةً الأفضل اتصالًا.
   (اقرأ [ملاحظة المناطق](#ملاحظة-المناطق) قبل أن تحسم — على خطة Netlify
   المجانية يقلّ أثر هذا الاختيار.)

انتظر حتى ينتهي التجهيز (~دقيقتان).

---

## ٢. تركيب قاعدة البيانات

من لوحة Supabase ← **SQL Editor** ← **New query**.

نفّذ الملفات الخمسة **بهذا الترتيب**، كل ملف في استعلام منفصل، واضغط
`Run` بعد كل واحد وتأكّد من نجاحه قبل الانتقال للذي بعده:

| # | الملف | ما يفعله |
|---|---|---|
| ١ | `supabase/migrations/0001_schema.sql` | ٩ جداول + الفهارس + القيود |
| ٢ | `supabase/migrations/0002_functions.sql` | `award_points`، الصلاحيات، إدارة الفصول |
| ٣ | `supabase/migrations/0003_rls.sql` | ٢٠ سياسة عزل + العرض العام `student_farms` |
| ٤ | `supabase/migrations/0004_seed.sql` | المجموعات السبع، الحدود، الفصل الأول، trigger المصادقة |
| ٥ | `supabase/migrations/0005_quadrant_layout.sql` | تخطيط الأرباع: كل فئة نبتات في بستانها |
| ٦ | `supabase/migrations/0006_undo_and_compact.sql` | تراجع المشرف خلال دقيقتين + ملء الخانة المُلغاة |

> **قاعدة قائمة نُفِّذت عليها 0001–0004 سابقًا؟** نفّذ `0005` وحده. يعيد رسم
> المزارع القائمة بالتخطيط الجديد في معاملة واحدة، ولا يمسّ النقاط ولا
> الترتيب — تتغيّر مواضع النبتات فقط.

> **قاعدة نُفِّذ عليها حتى 0005؟** نفّذ `0006` وحده. لا يحرّك أي نبتة قائمة:
> يضيف دالة التراجع، ويجعل المنح التالي يملأ خانة أي نبتة أُلغيت بدل أن
> تبقى فجوة. بدونه يعمل الموقع كله، ويظهر للمشرف عند ضغط «تراجع» أن الميزة
> تحتاج هذا الملف.

> **الترتيب ليس اختياريًا.** الملف ٢ يحتاج الأنواع المعرّفة في ١، والملف ٣
> يحتاج الدوال المعرّفة في ٢.

**تحقّق سريع** — نفّذ هذا وتأكّد أن كل سطر يُرجع `t`:

```sql
select
  (select count(*) from groups)              = 7  as groups_ok,
  (select count(*) from daily_limits)        = 2  as limits_ok,
  (select count(*) from semesters where is_active) = 1 as semester_ok,
  to_regprocedure('award_points(uuid,point_tier)') is not null as rpc_ok,
  (select x = 1 and y = 1 from quadrant_coord('green', 0)) as layout_ok,
  to_regprocedure('undo_my_award(bigint)') is not null as undo_ok;
```

---

## ٣. إنشاء حساب المدير

**Authentication** ← **Users** ← **Add user** ← *Create new user*:

- البريد وكلمة المرور: كما تريد (سجّلهما)
- **Auto Confirm User**: مفعّل ✅ (بدونه لن يستطيع الدخول)

ثم افتح المستخدم ← **User Metadata** ← والصق:

```json
{ "full_name_ar": "اسم المدير" }
```

الـ trigger ينشئ صفّه في `public.users` بدور `group_supervisor`. الدور لا
يُقرأ من `user_metadata` عن قصد — المفتاح العام موجود في حزمة المتصفح، ومن
يملكه يستطيع التسجيل ببيانات وصفية من اختياره — فالترقية تُصنع هنا، في
**SQL Editor**، مرة واحدة:

```sql
update public.users set role = 'admin'
 where id = (select id from auth.users where email = 'البريد الذي أنشأته');
```

> إن أنشأت الحساب **قبل** تنفيذ الملف `0004_seed.sql` فلن يُنشأ صفّه في
> `public.users` لأن الـ trigger لم يكن موجودًا بعد. أصلحه بهذا:
>
> ```sql
> insert into public.users (id, full_name_ar, role)
> select id, coalesce(raw_user_meta_data->>'full_name_ar', email), 'admin'
>   from auth.users
>  on conflict (id) do update set role = 'admin';
> ```

**التقط مفاتيح المشروع** من **Project Settings** ← **API**:

| المفتاح | أين تجده |
|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | Project URL |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | `anon` / `public` |
| `SUPABASE_SERVICE_ROLE_KEY` | `service_role` — **سرّي، خادمي فقط** |

> مفتاح `service_role` يتجاوز كل سياسات العزل. لا تضعه في أي متغيّر يبدأ
> بـ `NEXT_PUBLIC_`، ولا تلصقه في أي ملف داخل المستودع.

---

## ٤. النشر على Netlify

1. [app.netlify.com](https://app.netlify.com) ← **Add new site** ←
   *Import an existing project* ← اختر GitHub ومستودعك
2. **Base directory** ← اكتب **`sabbaq`**

   > ### هذه أهمّ خطوة
   > المستودع يحتوي مشروعًا آخر في جذره. بدون ضبط هذا الحقل سيبني Netlify
   > المشروع الخطأ. وهو أيضًا ما يجعل Netlify يجد `sabbaq/netlify.toml` —
   > فهو يبحث بالترتيب: مجلّد الحزمة ← المجلّد الأساسي ← الجذر.

3. **Build command** و **Publish directory**: اتركهما — `netlify.toml`
   يضبطهما (`npm run build` و `.next`).
   الترويسات الأمنية وتحويل `/screen` معرّفة في `next.config.ts` لا في
   `netlify.toml`، فيطبّقها Next نفسه وتعمل محليًا وعلى أي مستضيف.
4. **Environment variables** ← *Add a variable* ← أضِف الثلاثة من الخطوة ٣:

   ```
   NEXT_PUBLIC_SUPABASE_URL
   NEXT_PUBLIC_SUPABASE_ANON_KEY
   SUPABASE_SERVICE_ROLE_KEY
   ```

   اجعل نطاقها **All scopes** (البناء ووقت التشغيل معًا): متغيّرات
   `NEXT_PUBLIC_` تُدمَج في الحزمة وقت البناء، والثالث يُقرأ وقت التشغيل.

5. **Deploy site**

محوّل Next.js (`@netlify/plugin-nextjs`) يُركَّب تلقائيًا عند اكتشاف
Next.js — لا تُضِفه يدويًا ولا تثبّت نسخته. الوسيط (middleware) و
ISR/`revalidate` و Server Actions كلها مدعومة عليه، وهي ما يبني عليه سبّاق.

### ملاحظة المناطق

دوال Netlify تعمل افتراضيًا في **`cmh` (أوهايو، أمريكا)**، وتغيير المنطقة
متاح على خطّتي **Pro و Enterprise** من
*Cloud compute ← Functions ← Region* (ثم أعد النشر). المناطق المتاحة تشمل
`fra` (فرانكفورت) و `dub` و `lhr`.

ما يعنيه هذا عمليًا:

- **القراءة** (عرض المزارع، الصفحة العامة، وضع الشاشات) تُخزَّن على شبكة
  Netlify العالمية، فالطلاب والشاشات لا يتأثّرون.
- **الكتابة** (منح المشرف نقطة) تمرّ من الدالة إلى Supabase. على الخطة
  المجانية يعني ذلك رحلة من أوهايو إلى منطقة قاعدتك — تأخير ملموس وقت ذروة
  نهاية اليوم حين يمنح المشرفون معًا.

**التوصية:** إن بقيت على المجانية، اختر منطقة Supabase قريبة من
`us-east-1` لتقصير المسافة بين الدالة والقاعدة. وإن انتقلت إلى Pro، اضبط
منطقة الدوال على `fra` واجعل Supabase في `eu-central-1` — فتقترب الاثنتان
من المدرسة معًا.

---

## ٥. التحقّق بعد النشر

### فحص تمهيدي آلي

```bash
cd sabbaq
NEXT_PUBLIC_SUPABASE_URL=... NEXT_PUBLIC_SUPABASE_ANON_KEY=... npm run preflight
```

يتأكّد من: وصول القاعدة، المجموعات السبع، وجود فصل نشط، عمل `quadrant_coord`،
قابلية `student_farms` للقراءة العامة، ورفض الكتابة على الزائر.

### بيانات تجريبية (موصى به)

لتجربة الرحلة كاملة قبل إدخال طلاب حقيقيين، نفّذ في SQL Editor:

```
supabase/seed/demo.sql
```

يُنشئ ٤٢ طالبًا في المجموعات السبع بمزارع متفاوتة النمو (من فارغة إلى ٢٨٠
نبتة ≈ فصل كامل)، وينسب النقاط لحساب المدير. لحذفها لاحقًا:

```sql
delete from students where full_name like '[تجريبي]%';
```

### فحص يدوي — الرحلة كاملة

| # | الخطوة | المتوقّع |
|---|---|---|
| ١ | افتح `/` | شبكة المجموعات السبع + مربّع بحث |
| ٢ | ابحث عن طالب واختره | تُفتح `/farm/<id>` والمزرعة تُرسَم مؤطَّرة |
| ٣ | اسحب المزرعة وكبّرها | حركة سلسة، السياج يحيط بها |
| ٤ | افتح `/tv` (أو `/screen`) | تدوير تلقائي بين المتميّزين |
| ٥ | افتح `/supervisor` بمتصفّح خفي | تحويل إلى `/login` |
| ٦ | ادخل بحساب المدير | لوحة المشرف تظهر |
| ٧ | اختر طالبًا واضغط `+50` | إشعار نجاح وارتفاع رصيده |
| ٨ | افتح مزرعة ذلك الطالب بمتصفّح خفي | النبتة الحمراء الجديدة موجودة |
| ٩ | `/admin` ← الحدود ← اضبط حدًّا صغيرًا | يُحفظ فورًا |
| ١٠ | أنشئ مشرف مجموعة وادخل بحسابه | يرى مجموعاته فقط، والأزرار تُعطَّل عند الحد |

الخطوتان ٧ و٨ معًا هما الاختبار الحقيقي: المنح يكتب في قاعدة البيانات
والقراءة العامة تعرضه بلا حساب.

---

## مشاكل شائعة

| العرض | السبب والحل |
|---|---|
| صفحة «سبّاق غير مهيّأ بعد» | متغيّر بيئة ناقص أو خطأ حرف في اسمه. الوسيط يسمّي الناقص بالاسم. أضِفه في Netlify ثم **أعد النشر** — المتغيّرات لا تُطبَّق على نشر قائم. |
| البناء يفشل أو يُنشَر مشروع غريب | **Base directory** غير مضبوط على `sabbaq`. |
| البناء ينجح لكن الصفحات تُرجع 404 | `netlify.toml` لم يُقرأ لأن Base directory غير مضبوط، فلم يُطبَّق `publish = ".next"`. |
| الدخول يقول «بيانات غير صحيحة» والحساب موجود | *Auto Confirm User* لم يكن مفعّلًا. فعّله من Authentication ← Users. |
| الدخول يقول «تعذّر الوصول إلى الخادم» | الرسالة تعني أن الخطأ **ليس** في كلمة المرور: `NEXT_PUBLIC_SUPABASE_URL` أو `NEXT_PUBLIC_SUPABASE_ANON_KEY` خاطئ. تحقّق من مطابقة معرّف المشروع في الرابط لما في لوحة Supabase حرفًا بحرف. |
| المفتاح صحيح في اللوحة لكنه لا يعمل | نُسخ **النص المعروض** لا القيمة: لوحة Supabase تعرض المفتاح مقنّعًا (`eyJhbGci••••…`)، وتحديده بالماوس ينسخ النقاط حرفيًا. استخدم زر **Copy**. مفتاح سليم يحتوي نقطتين فاصلتين `.` ولا يحتوي `•` إطلاقًا. |
| الدخول ينجح ثم «حسابك غير مرتبط بدور» | لا صفّ في `public.users`. نفّذ استعلام الإصلاح في الخطوة ٣. |
| منح النقاط يفشل بـ `NO_ACTIVE_SEMESTER` | لا فصل نشط. أنشئه من `/admin/semesters` أو: `insert into semesters (name_ar, name_en, start_date, end_date, is_active) values ('الفصل الأول','Semester 1', current_date, current_date + 90, true);` |
| المزرعة فارغة والطالب له نقاط | نقاطه في فصل غير النشط. `student_farms` تعرض الفصل النشط وحده. |
| المزرعة صفحة زرقاء بلا نبتات | WebGL معطّل في المتصفح، أو تعجيل الرسوم مطفأ على جهاز العرض. |
| منح النقاط بطيء (> ثانيتين) | منطقة الدوال بعيدة عن قاعدتك — انظر [ملاحظة المناطق](#ملاحظة-المناطق). |

---

## ملاحظات تشغيلية

- **التخزين المؤقت — يعمل على الصفحات العامّة.** `/` و `/tv` ساكنتان
  (تحديث كل ٦٠ ثانية)، و `/farm/[id]` تُولَّد عند أول طلب ثم تُخزَّن (٣٠
  ثانية). تحقّق بنفسك: `npm run build` يطبع `○` أمام الأوليين و `●` أمام
  الثالثة، و `ƒ` أمام صفحات الموظّفين وحدها — وهي ديناميكية بحقّ لأن فيها
  جلسة. وعلى الموقع الحيّ تظهر الإصابة في الترويسات:
  `cache-status: "Netlify Durable"; hit`.

  هذا لم يكن يعمل سابقًا: ثلاثة أشياء كانت تُسقطه بصمت — قراءة كوكي اللغة
  في التخطيط الجذر، وعميل Supabase الذي يقرأ الكوكيز ليحمل جلسة، والوسيط
  الذي كان يعمل على كل مسار. إن أعدتَ أيًّا منها إلى صفحة عامّة عاد
  `ƒ` وعاد `no-store` معه، فانتبه لذلك عند التعديل.

  اللغة لا تُجزّئ الكاش: `netlify-vary` لا يشمل كوكي اللغة، وطلبٌ بلا كوكي
  وآخر بـ `ar` وثالث بـ `en` تصيب الكاش ثلاثتها.
  وضع الشاشات يُعيد الجلب كل ٥ دقائق من المتصفح، وهذا يعمل كما وُصف.
- **شاشات المدرسة**: افتح `/tv` بملء الشاشة — أو `/screen` وهو أقصر
  للكتابة على جهاز بلا لوحة مفاتيح. الرابط ثابت ولا يحتاج حسابًا، واللغة
  محفوظة في كوكي لسنة فيبقى إعداد الشاشة كما ضُبط.
- **النسخ الاحتياطي**: الخطة المجانية في Supabase تحفظ نسخًا يوميًا لسبعة
  أيام. سجل النقاط هو المصدر الوحيد للحقيقة — لا شيء آخر يُعاد بناؤه منه.
- **قبل نهاية الفصل**: أرشِف من `/admin/semesters` قبل أي تصفير. الأرشفة
  تحفظ لقطة جاهزة للتقارير؛ التصفير بدونها يُبقي السجل الخام فقط.
- **النطاق المخصّص**: *Domain management ← Add a domain*. شهادة TLS
  تُصدَر تلقائيًا.
